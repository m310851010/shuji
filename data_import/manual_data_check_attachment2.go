...
// 查询附件2数据，指定数据库文件路径
func (s *DataImportService) QueryDataDetailAttachment2ByDBFile(obj_ids []string, dbFilePath string) db.QueryResult {
	if len(obj_ids) == 0 {
		return db.QueryResult{
			Ok:      false,
			Message: "没有提供obj_id",
	...
	for _, obj_id := range obj_ids {
		result := s.queryDataDetailAttachment2Forinner(obj_id, database)
