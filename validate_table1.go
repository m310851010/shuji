...
func (a *App) validateEnergyDevice(row map[string]interface{}, rowNumber int, errors *[]ValidationError) {
	...
	if !table1EquipTypeMap[equipType] {
		*errors = append(*errors, ValidationError{ Type: ERROR_TYPE_OPTION_ERROR, Message: "类型不在选项中", Flag: 0, Cells: GetCellPosition(equipFieldMapping, rowNumber, "equip_type",), RowNumber: rowNumber,})
	}
	// 能效水平
	energyEfficiency := getStringValue(row["energy_efficiency"]) // 能效水平
	if !table1EnergyEfficiencyMap[energyEfficiency] {
		*errors = append(*errors, ValidationError{ Type: ERROR_TYPE_OPTION_ERROR, Message: "能效水平不在选项中", Flag: 0, Cells: GetCellPosition(equipFieldMapping, rowNumber, "energy_efficiency",), RowNumber: rowNumber,})
	}
	// 容量单位
	capacity_unit := getStringValue(row["capacity_unit"]) // 容量单位
	if !table1CapacityUnitMap[capacity_unit] {
		*errors = append(*errors, ValidationError{ Type: ERROR_TYPE_OPTION_ERROR, Message: "容量单位不在选项中", Flag: 0, Cells: GetCellPosition(equipFieldMapping, rowNumber, "capacity_unit",), RowNumber: rowNumber,})	
		}
	// 耗煤品种
	coalProduct := getStringValue(row["coal_type"]) // 耗煤品种
	if !table1CoalProductMap[coalProduct] {
		*errors = append(*errors, ValidationError{ Type: ERROR_TYPE_OPTION_ERROR, Message: "耗煤品种不在选项中", Flag: 0, Cells: GetCellPosition(equipFieldMapping, rowNumber, "coal_type",), RowNumber: rowNumber,})	
	}
}
...
func (a *App) validateEnergyPurposeDetail(row map[string]interface{}, rowNumber int, errors *[]ValidationError) {
	...
	if exists && creditCodeStatDate.Annual_raw_coal > 0 && main_usage != "原料" {
	 ...
	 var usage map[string]map[string]map[string]bool
	 var specific map[string]map[string]bool
	 var types map[string]bool
	if main_usage != "" {
		usage, exists = mainUsageMapping[main_usage]
		if !exists {
			*errors = append(*errors, ValidationError{ Type: ERROR_TYPE_OPTION_ERROR, Message: "主要用途不在选项中", Flag: 0, Cells: GetCellPosition(usageFieldMapping, rowNumber, "main_usage" ), RowNumber: rowNumber,})		
			return;	
		}
	}
	...
	if specific_usage != "" {
		...
		if  !exists {
			*errors = append(*errors, ValidationError{ Type: ERROR_TYPE_OPTION_ERROR, Message: "具体用途不在选项中", Flag: 0, Cells: GetCellPosition(usageFieldMapping, rowNumber, "specific_usage"), RowNumber: rowNumber,})
			return;
		}
	}
	...
	if output_energy_types != "" {	
		...
		if  !exists {
			*errors = append(*errors, ValidationError{ Type: ERROR_TYPE_OPTION_ERROR, Message: "产出品种品类不在选项中", Flag: 0, Cells: GetCellPosition(usageFieldMapping, rowNumber, "output_energy_types"), RowNumber: rowNumber,})	
			return;
		}
	}
	...
	if measurement_unit != "" {
		if _, exists = types[measurement_unit]; !exists {
			*errors = append(*errors, ValidationError{ Type: ERROR_TYPE_OPTION_ERROR, Message: "产出计量单位与产出品种品类不一致", Flag: 0, Cells: GetCellPosition(usageFieldMapping, rowNumber, "measurement_unit"), RowNumber: rowNumber,})	
			return;
		}
	}
}
...
func (a *App) validateEnergyUnit(fieldMapping map[string]ExcelFieldMapping, row map[string]interface{}, rowNumber int, flag int, fieldNames []string, errors *[]ValidationError) {
	for _, fieldName := range fieldNames {
		...
		if parseFloat(value) / 10000 > 1 {
			*errors = append(*errors, ValidationError{ Type: ERROR_TYPE_DATA_UNIT_MISMATCH, Message: "数据与单位不匹配", Flag: flag, Cells: GetCellPosition(fieldMapping, rowNumber, fieldName,), RowNumber: rowNumber,})
		}
	}
}
...
func (a *App) validateEnergyConsumptionTotal(row map[string]interface{}, rowNumber int, errors *[]ValidationError) {
	...
	if sumCoalConsumption != annualTotalCoalConsumption {
		*errors = append(*errors, ValidationError{ Type: ERROR_TYPE_DATA_LOGIC_ERROR, Message: "耗煤总量（实物量）与原煤消费量、洗精煤消费量、其他煤炭消费量（实物量）加和不相等", Flag: 1, Cells: GetCellPosition(mainFieldMapping, rowNumber, "annual_total_coal_consumption", "annual_raw_coal_consumption", "annual_clean_coal_consumption", "annual_other_coal_consumption"), RowNumber: rowNumber,})
	}
}
...
func (a *App) validatePhone(row map[string]interface{}, rowNumber int, errors *[]ValidationError) {
	...
	if !IsPhone(tel) {
		*errors = append(*errors, ValidationError{ Type: ERROR_TYPE_TEXT_FORMAT_ERROR, Message: "应为手机号或固话", Flag: 0, Cells: GetCellPosition(mainFieldMapping, rowNumber, "tel"), RowNumber: rowNumber,})
	}
}
...
func (a *App)validateStatDate(fieldMapping map[string]ExcelFieldMapping, row map[string]interface{}, rowNumber int, errors *[]ValidationError) {
	...
	if statDate == "" {
		*errors = append(*errors, ValidationError{ Type: ERROR_TYPE_TEXT_FORMAT_ERROR, Message: "不能为空", Flag: 0, Cells: GetCellPosition(fieldMapping, rowNumber, "stat_date"), RowNumber: rowNumber,})
	} else {
		// 请严格填写四位数字并使用常规字体，不得使用宽体字符，如“2023”“2024”，不得填写为“2023年”“2024年”。
		isNumber := isInteger(statDate)
		if !isNumber || len(statDate) != 4 {
			*errors = append(*errors, ValidationError{ Type: ERROR_TYPE_TEXT_FORMAT_ERROR, Message: "应为四位数并使用常规字体", Flag: 0, Cells: GetCellPosition(fieldMapping, rowNumber, "stat_date"), RowNumber: rowNumber,})
		}
	}
}
...
func (a *App) validateTrade(fieldMapping map[string]ExcelFieldMapping, row map[string]interface{}, rowNumber int, errors *[]ValidationError) {
	...
	if _, exists := tradeMapping[trade_a]; !exists {
		*errors = append(*errors, ValidationError{ Type: ERROR_TYPE_OPTION_ERROR, Message: "不在行业门类选项中", Flag: 0, Cells: GetCellPosition(fieldMapping, rowNumber, "trade_a"), RowNumber: rowNumber,})	
		return;	
	}
	if _, exists := tradeMapping[trade_a][trade_b]; !exists {
		*errors = append(*errors, ValidationError{ Type: ERROR_TYPE_OPTION_ERROR, Message: "行业门类和行业大类不一致", Flag: 0, Cells: GetCellPosition(fieldMapping, rowNumber, "trade_b"), RowNumber: rowNumber,})
		return;
	}
	if _, exists := tradeMapping[trade_a][trade_b][trade_c]; !exists {
		*errors = append(*errors, ValidationError{ Type: ERROR_TYPE_OPTION_ERROR, Message: "行业大类和行业中类不一致", Flag: 0, Cells: GetCellPosition(fieldMapping, rowNumber, "trade_c"), RowNumber: rowNumber,})
	}
}
