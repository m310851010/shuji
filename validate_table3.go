...
func (a *App) validateTable3(filePath string) QueryResult {
...
func (a *App)  validateTrade3(row map[string]interface{}, rowNumber int, errors *[]ValidationError) {
	...
	if len(trade_a) != 2 || !isInteger(trade_a) {
		*errors = append(*errors, ValidationError{ Type: "required", Message: "应为2位代码", Flag: 0, Cells: GetCellPosition(table3FieldMapping, rowNumber, "trade_a"), RowNumber: rowNumber,})
	}

	if len(trade_c) != 4 || !isInteger(trade_c) {
		*errors = append(*errors, ValidationError{ Type: "required", Message: "应为4位代码", Flag: 0, Cells: GetCellPosition(table3FieldMapping, rowNumber, "trade_c"), RowNumber: rowNumber,})
	}
}
...
func (a *App) validateIsSubstitution(row map[string]interface{}, rowNumber int, errors *[]ValidationError) {
	...
	if is_substitution != "是" && is_substitution != "否" {
		*errors = append(*errors, ValidationError{ Type: "required", Message: "应为“是”或“否”", Flag: 0, Cells: GetCellPosition(table3FieldMapping, rowNumber, "is_substitution"), RowNumber: rowNumber,})
	}
}
